...
class IndexedCorpus:
    def __init__(self, data_path, dictionary, dataset_limit=None):
        ...
        if dataset_limit is not None and len(self.indexed_documents) >= dataset_limit:
            self.indexed_documents = self.indexed_documents[:dataset_limit]
...
class PairedDataset:
    ...
    def __init__(self, positive_data_path, negative_data_path, dictionary, dataset_limit=None):
        self.positive_source_corpus = MaskedCorpus(positive_data_path, dictionary, dataset_limit=dataset_limit)
        self.negative_source_corpus = MaskedCorpus(negative_data_path, dictionary, dataset_limit=dataset_limit)
        self.dataset_size = self.positive_source_corpus.sentences_count + self.negative_source_corpus.sentences_count
        self.corpus_size = len(self.positive_source_corpus) + len(self.negative_source_corpus)

    def __getitem__(self, item):

        positive_document_index = randint(0, len(self.positive_source_corpus)-1)
        positive_document = self.positive_source_corpus[positive_document_index]
        positive_sentence_index = randint(0, len(positive_document) - 2)
        A_masked_sentence, A_target_sentence = positive_document[positive_sentence_index]

        if random() < 0.5:  # 50% of the time B is the actual next sentence that follows A
            B_sentence_index = randint(0, len(positive_document) - 2)
            B_masked_sentence, B_target_sentence = positive_document[B_sentence_index + 1]
            is_next = 1
        else:  # 50% of the time it is a random sentence from the corpus
            random_document_index = randint(0, len(self.negative_source_corpus)-1)
            random_document = self.negative_source_corpus[random_document_index]
